...
tools/sentinel-worker/
  ...
  jobs/*.go (prompt consts)  # prompt text: inline Go string consts, NOT prompts/*.tmpl (deliberate,
                             # see §10 "Inline-const prompts, not go:embed templates")
...
`KEY=VALUE` or bare `KEY` entries feeding `jobs.FixRunner.ExecutorEnv`, the ONLY channel through
which `$FIX_EXECUTOR_CMD` receives ITS OWN credentials, e.g. a droid/DeepSeek API key; every key is
validated at config-load time against the same forbidden-key guard `buildExecutorEnv` enforces at
runtime — `SENTINEL_AGENT_KEY`/`LLM_*_API_KEY`/git tokens are rejected, never passed through),
...
- **N8f FIX pipeline**: unit/integration coverage is stub-executor-based
  ...
  **Manual verification recipe with a real executor (Factory `droid`, DeepSeek BYOK — the first
  ...
  2. Set `FIX_EXECUTOR_CMD` to droid's non-interactive invocation, e.g.
     ...
     the command to read `$TASK_MD`, not stdin. Set `WORKER_FIX_EXECUTOR_ENV=DEEPSEEK_API_KEY=<key>`
     (fix-lifecycle remediation round 2, finding 4) to hand droid its own DeepSeek BYOK credential —
     this is the ONLY channel that reaches the executor's child env; the worker's own
     `SENTINEL_AGENT_KEY`/`LLM_*_API_KEY`/git tokens are never in scope for it, and a
     `WORKER_FIX_EXECUTOR_ENV` entry naming one of them is rejected at config-load time.
...
| **N8h — DONE 2026-08-21** (holistic-review remediation) | Closes the 22 findings from the Fable holistic review across four groups — FIX-subsystem, git-security, advisor-spend-input, durability-startup — every one an instance of the repo's B3 "config read+validated but never consumed / unit-tested on hand-seeded state the real pipeline never produces" failure. Highlights: **§2.8 S3 state-snapshot durability made real and wired** (`state/s3snapshot.go` hand-rolled SigV4 PUT/GET; `snapshotManager` restore-on-empty-startup + periodic + post-compaction + SIGTERM-upload, all from `runPipeline`/`runWorker` — at N8g only the interface + local fallback existed); **`WORKER_DAILY_TOKEN_BUDGET`/`WORKER_MAX_TRIAGE_PER_HOUR` wired into `loop.Runner`** (`llm.DailyBudget`/`HourlyCounter` gate every Advisor call, seeded from the journal so "per day" survives a restart); **all §7 metrics completed** (in-lane retries, circuit-open events, per-scope circuit-state gauge, fix attempts, PRs opened, heartbeats, gate rejections, llm-tokens-by-provider, budget-remaining, plus separated bootstrap enqueued/skipped counters); FIX job gets a **distinct `state.JobID(FixKind,…)`** (BLOCKER: previously reused the parent triage jobID, whose terminal-guard silently dropped every FIX journal record); **askpass host-pin now includes the port** and the branch push routes through `RunGitWithHost` with an explicitly-derived expectedHost; **per-project PR cap** (`ProjectSettings.MaxPRsPerDay`, C15) threaded into `FixCaps.AllowPR`; **git-auth-failure forces an immediate settings re-fetch** (C16); **`HasOpenQuestion` fixed** to survive a triage job's own post-Act terminal tail; **startup free-disk guard** on `WORKER_STATE_DIR` (flips readiness, not just logs); **`WORKER_EVENT_TYPES` validated** against the dispatcher's real vocabulary; **workspace orphan sweep + retention reaper** wired from `runPipeline`; **tool-result fencing** — every mid-loop tool SUCCESS result now `guard.WrapUntrusted`-fenced (previously only the pre-loaded system prompt was). Every guard mutation-tested (delete the production line → the real-path test goes red). Worker module gate green (`GOWORK=off build/vet/gofmt/test -race`, 925 test cases, 12 packages). e2e U40/U41/U42 re-run against the live compose stack (dashboard :13000, ingestor :18080) → all PASS, zero skips under `SENTINEL_E2E=1`; U41 needed no change. | worker module gate green incl. `-race`+`gofmt`; reachability sweep confirms every remediated capability is reachable from `main()` (not tested-but-unwired); real-path wiring tests `TestRunApp_S3SnapshotBackend_{UploadsOnSIGTERMBeforeExit,RestoresOnEmptyStateDirStartup}`, `TestBuildRunner_WiresDailyBudgetAndTriageLimiterFromConfig`, `TestGuard_OnRejectionHook_FiresOnRealCheckRejection`, `TestRunPipeline_WiresFixRunnerAndSeedsCapsFromJournal`, `TestCheckStateDirFreeDisk_RefusesBelowMinimum`/`TestRunWorker_LowDiskFlipsReadinessUnhealthy`, `TestLoadConfig_RejectsUnknownEventType`; S3 wire protocol proven against a SigV4-verifying httptest fake (`state/s3snapshot_test.go`); `SENTINEL_E2E=1 DASHBOARD_URL=http://localhost:13000 INGESTOR_URL=http://localhost:18080 go test -tags=e2e ./tests/e2e/ -run 'TestU40_\|TestU41_\|TestU42_'` → 3 PASS. **Still deferred (unchanged from N8g, not a regression):** a real cluster/minio S3 round-trip and a real K8s-Secret PATCH stay interface-and-fake-proven only; U43 (FIX binary end-to-end) stays a `jobs`-level integration test, not `tests/e2e`, because `$FIX_EXECUTOR_CMD` needs a non-CI-installable coding CLI. |
| **N8i — DONE 2026-08-21** (holistic-review remediation, round 2) | Closes a deeper tier the second Fable pass surfaced, **including two regressions the N8h changes had introduced**, again all instances of the B3 "config/field read-and-validated but never consumed on the real pipeline path" family. Landed in three groups. **Group A — act-phase honesty:** `checkBatchResults` now consults `sentinel.ClassifyBatch`'s **envelope** class (`perOp == nil` ⇒ a 401/429/5xx batch envelope is surfaced as a real transient/auth failure, not silently treated as an empty-but-successful per-op walk that lost the whole decision); the act-phase in-lane retry ladder + verbatim narrow re-send (`Runner.runWithInlaneRetry` re-driving `Act.Act`, which **replays the journaled batch verbatim** from the `StateActing` payload — Advisor never re-consulted); `RealActor.journalQuestioned` writes a `StateQuestioned` record so `HasOpenQuestion` is satisfiable end-to-end (feeds the dispatch `hasOpenQuestion` arm and `sweep.ReconcileReaped`). **Group B — dispatch/journal durability:** redelivery dedupe in `Dispatcher.Enqueue` (a re-delivered event whose jobId has ANY terminal record is dropped **before** the queued record is appended, so it can't blind `Runner.Run`'s own terminal-dedupe); the `skip-if-fix-in-flight` rule (`Classify`'s `hasOpenFix` seam wired from `PollLoop.hasOpenFix` → `Journal` open-fix scan); **FIX terminal journaling on ALL exit paths** (`journalFixTerminal` in `releaseWithComment` — the propose-only / caps-exhausted / resolve-repo-error / no-repo-connection paths previously returned after `journalFixRunning` without ever writing a terminal record, so `RecoveryScan` re-surfaced and re-failed the identical dead job every restart — **regression fixed**). **Group C — startup/config/seed:** async bounded boot recovery for in-flight FIX (`resumeInFlightJob` → `FixRunner.DispatchResume` on its own timeout-bounded goroutine — N8h had it call `ResumeFix` synchronously on `runPipeline`'s long-lived ctx, so one stale FIX attempt blocked the worker from ever polling or serving `/readyz` — **regression fixed**); **separate circuit breakers** for the write-path (`ScopeSentinelAPI`) and the poll read-path (`ScopeSentinelAPIPoll`), reverting N8h Group D's "share one breaker" (whose side effect was the poll loop's frequent `RecordSuccess` resetting the runner's consecutive-failure streak so the write-path circuit could never open) while **keeping both scopes' gauges** (`circuit_state_<scope>`, each with its own `publishCircuitStateGauge` refresher — N8h's actual goal was gauge visibility, preserved); `WORKER_MAX_TRIAGE_PER_HOUR`'s `HourlyCounter` now **seeded from the journal** (`SumTriageStarts`) like `DailyBudget`/`FixCaps` already were (a crash-loop within one UTC hour no longer resets the hourly cap to zero every restart); `cursor_lag` gauge now accumulates a WHOLE PollOnce drain cycle's event count (`OnPageDrained`) instead of only the tail page's; repoctx periodic `git fetch` now pins the askpass host via `RunGitWithHost`/`expectedRefreshHost` derived from the known-good clone URL (a bare `git fetch origin` carried no URL for `deriveExpectedHost`, so the pin was silently DISABLED on every refresh); config validation extended (`WORKER_FIX_EXECUTOR_ENV` forbidden-key rejection). **New env `WORKER_FIX_EXECUTOR_ENV`** (finding 4): the previously-missing channel feeding `FixRunner.ExecutorEnv` — without it the Fix Executor (`$FIX_EXECUTOR_CMD`, e.g. droid) received NO credentials at all (the forbidden-key guard in `jobs/fix_executor.go` was guarding a field nothing ever populated), so an operator could not hand it a DeepSeek/model BYOK key; §5/§8/`.env.example` document it. Every guard mutation-tested. Worker module gate green (`GOWORK=off build/vet/gofmt/test -race`, 12 packages). e2e U40/U41/U42 re-run against the live compose stack (dashboard :13000, ingestor :18080) → all PASS, zero skips under `SENTINEL_E2E=1`; U41 already exercises the needs_info/question kill-9 replay path (exactly-1 blocking question, Advisor-not-re-consulted, severity replays, claim kept, `waiting_on='reporter'`, reply drives FOLLOW-UP) and needed no change. | worker module gate green incl. `-race`+`gofmt`; reachability sweep confirms all 14 remediated capabilities reachable from `main()` (batch envelope classification, act-phase in-lane retry + narrow re-send, question-status check, replay-verbatim, redelivery dedupe, FIX terminal journaling on all paths, async bounded boot recovery, skip-if-fix-in-flight, executor env channel, separate-breakers-both-gauges, config validation, hourly seed, cursor_lag, repoctx fetch pin) — none tested-but-unwired; real-path wiring/mutation tests incl. `TestBuildFixRunner_ExecutorEnvReachesTheRealExecutorChild`, `TestResumeInFlightJob_SlowFixRunsAsyncAndDoesNotBlockCaller`, `TestResumeInFlightJob_FixRunning_DrivesResumeFix`, `TestSeparateBreakers_RunnerOpensWhileConcurrentPollLoopKeepsSucceeding`, `TestRunApp_CursorLagReflectsWholeCycleBacklogNotLastPage`, `TestBuildRunner_SeedsTriageLimiterFromJournaledClaimedRecords`, `TestCache_RefreshFetch_InsteadOfRedirect_NoAuthLeaksToAttackerHost`, `TestRealActor_Act_Replay_ResendsJournaledBatchVerbatim`, `TestCheckBatchResults_NonSuccessEnvelope_{401IsAuth,429IsTransient,5xxIsTransient}`, `TestRunFix_{ValidationFailure,CapsExhausted}_JournalsTerminal`, `TestDispatcher_EnqueueDropsRedeliveredTerminalJob`, `TestClassify_OccurrenceBurstSuppressedByOpenFix`, `TestLoadConfig_WorkerFixExecutorEnv_RejectsForbiddenKey`; `SENTINEL_E2E=1 DASHBOARD_URL=http://localhost:13000 INGESTOR_URL=http://localhost:18080 go test -tags=e2e ./tests/e2e/ -run 'TestU40_\|TestU41_\|TestU42_'` (workspace mode — e2e imports the local SDK) → 3 PASS. **Still deferred (unchanged, not a regression):** real cluster/minio S3 round-trip + real K8s-Secret PATCH stay interface-and-fake-proven; U43 (FIX binary end-to-end) stays a `jobs`-level integration test because `$FIX_EXECUTOR_CMD` needs a non-CI-installable coding CLI. |
| **N8j — DONE 2026-08-21** (holistic-review remediation, round 3, FINAL) | Third and last Fable pass; blocker count converged **4 → 2 → 0** — N8j carries **zero blockers**, closing the round-3 majors/minors with no new regressions. Two groups. **Group 1 — core robustness (loop/act):** `StateRetryableFailed` with a **non-empty** payload now routes to `resumeFromAdvised` in BOTH `Run`'s replay guard and `Resume`'s switch — the journaled Advisor decision is **replayed verbatim, Advisor never re-invoked** (empty-payload retryable_failed still full-re-runs); `checkBatchResults`' per-op default branch wraps `&sentinel.StatusError{Status: statusForOpClass(c)}` (maps `FailureClass`→representative HTTP status) so a **permanent per-op failure journals `failed_permanent` instead of being retried 5× as transient**; plus FIX findings on shared paths — `issues.progress` uses `message_md` (server 400'd on `body_md`), PROGRESS.md tail lines forwarded as `issues.progress`+journaled, per-connection `AgentCmd` (C15) overrides global `$FIX_EXECUTOR_CMD`, `CheckoutRelease` fetch host-pinned via `RunGitWithHost`. **Group 2 — FIX subsystem (gated OFF; real bugs when enabled):** `journalFixRunning` moved to `RunFix`/`ResumeFix` **entry (before workspace prep)** so a crash during prep is recoverable, not a silently-held claim; `journalFixRunning` gained `resumed bool` (→`FixRunningPayload.Resumed`) so `SeedToday` doesn't double-count crash-resumes against `WORKER_MAX_FIX_ATTEMPTS`; **Fix Executor's own BYOK cred** (`cfg.WorkerFixExecutorEnv` values) now fed to the log redactor's `configuredSecrets` (masked in logs/agent log); `AllowPR(*int)` — explicit `MaxPRsPerDay==0` **blocks unconditionally** (zero PRs), `nil`=unset→global cap; per-issue FIX attempt cap **resets at UTC midnight**; §7 volume caps gained `Remaining()` gauges (`fix_jobs_remaining`, `prs_remaining`, `triage_per_hour_remaining`, `followup_per_hour_remaining` — last wired to N8i's `WORKER_MAX_FOLLOWUP_PER_HOUR`), published at `buildFixRunner`/`buildRunner` and refreshed on `attempt-started`/`pr-opened`/`OnUsage`. Every guard red-first/mutation-tested. **Merge posture: no round 4** — TRIAGE/FOLLOW-UP core is live-proven and production-ready; FIX ships **gated experimental** until exercised against a real coding CLI. | worker module gate green (`GOWORK=off gofmt/build/vet/test -race`, 12 packages, independently re-run); reachability sweep — `buildFixRunner`/`buildRunner` reached from `runPipeline`, `TestRunPipeline_WiresFixRunnerAndSeedsCapsFromJournal` fails if unreachable, finding-6 mutation note proves redaction is live; red-first tests incl. `TestRunner_ActStageRetryableFailed_ResumeReplaysDecisionWithoutReinvokingAdvisor`, `TestCheckBatchResults_PerOpPermanentIsNotTransient`, `TestRunFix_JournalsFixRunningBeforeWorkspacePrep`, `TestFixCaps_SeedToday_ResumedRecordDoesNotDoubleCountAttempt`, `TestFixRunner_ExecutorEnvSecret_RedactedInAgentLog`, `TestFixCaps_AllowPR_ExplicitZeroProjectLimitBlocksUnconditionally`, `TestFixCaps_IssueAttempts_ResetsAtUTCMidnight`, `TestFixCaps_FixJobsRemainingAndPRsRemaining`; `SENTINEL_E2E=1 DASHBOARD_URL=http://localhost:13000 INGESTOR_URL=http://localhost:18080 NATS_URL=nats://localhost:4222 go test -tags=e2e ./tests/e2e/ -run 'TestU4[012]_Worker'` → 3 PASS (7.03s/5.19s/5.11s), 0 skips. **Still deferred (unchanged):** real cluster/minio S3 round-trip + real K8s-Secret PATCH stay interface-and-fake-proven; U43 needs a non-CI-installable coding CLI. |
...
**N8h (2026-08-21): holistic-review remediation — the last class of B3 seams closed.** The Fable
holistic review found 22 findings, every one the repo's characteristic shape: a config value or a
subsystem that was *read and validated and unit-tested*, but never *consumed by the running
pipeline* (or tested only on hand-seeded journal/decision state the real path never produces). The
worst offenders were seams that N8a–N8g's own phase notes had implicitly assumed live. Concretely
resolved and re-wired end-to-end from `main()` (`runApp`/`runWorker`/`runPipeline`/`buildRunner`/
`buildFixRunner`), each with a red-first real-path test:
- **§2.8 S3 snapshots were interface-only at N8g.** `state/s3snapshot.go` (hand-rolled SigV4) plus
  `main.go`'s `snapshotManager` now perform restore-on-empty-startup (before the journal is opened),
  periodic upload every `WORKER_SNAPSHOT_INTERVAL`, an upload after each journal compaction, and one
  final upload on SIGTERM before exit. Proven through the real entry path by
  `TestRunApp_S3SnapshotBackend_UploadsOnSIGTERMBeforeExit`/`RestoresOnEmptyStateDirStartup`; the
  wire protocol by a SigV4-verifying httptest fake. (A real cluster/minio round-trip stays deferred,
  same honest caveat as N8g.)
- **`WORKER_DAILY_TOKEN_BUDGET` + `WORKER_MAX_TRIAGE_PER_HOUR` had zero production callers.** Now
  `llm.DailyBudget`/`HourlyCounter` are constructed once in `buildRunner`, seeded from the journal
  (so "per day" survives a restart, not a per-process reset), and gate every Advisor invocation
  inside `loop.Runner.Run` before the LLM is ever called
  (`TestBuildRunner_WiresDailyBudgetAndTriageLimiterFromConfig`).
- **The FIX job reused the parent triage/followup jobID (BLOCKER).** The runner drives that jobID
  terminal immediately after `Act`, and `Journal.Append`'s terminal-guard then silently dropped every
  subsequent FIX record — so `journalFixRunning`/`JournalFixPROpen` never persisted and every FIX
  batch idempotency key collided with the parent's ops. `RealActor.Act` now derives a distinct
  `state.JobID(FixKind, issueID, triggerSeq)`.
- **Askpass host-pin ignored the port**, letting an `insteadOf` rewrite to the same host on a
  different port pass the pin; it now matches host AND port, and the branch push routes through
  `RunGitWithHost` with an explicitly-derived expectedHost (a bare `push` carries no URL in its args).
- **Tool results reached the model unfenced.** `guard.WrapUntrusted` previously only fenced content
  pre-loaded into the system prompt; anything a mid-loop `search_code`/`read_file`/`get_issue` call
  fetched went to the model raw. `recording()` in `jobs/toolchain.go` now fences every SUCCESS result.
- Plus: per-project PR cap (`ProjectSettings.MaxPRsPerDay`→`FixCaps.AllowPR`), git-auth-failure
  forcing an immediate settings re-fetch (C16 `OnAuthFailure`), `HasOpenQuestion` surviving a triage
  job's own post-Act terminal tail, a startup free-disk guard on `WORKER_STATE_DIR` that flips
  readiness, `WORKER_EVENT_TYPES` validated against the dispatcher's real vocabulary, the workspace
  orphan-sweep + retention reaper wired from `runPipeline`, and the full §7 metric set completed
  (in-lane retries, circuit-open events, per-scope circuit-state gauge, fix attempts, PRs opened,
  heartbeats, gate rejections, llm-tokens-by-provider, budget-remaining, separated bootstrap
  enqueued/skipped). Worker module gate green incl. `-race`/`gofmt` (925 test cases, 12 packages);
  e2e U40/U41/U42 green against the live stack, zero skips.
...
- **Inline-const prompts, not go:embed templates** (circuit-config-sec finding 8, plan doc
  correction — no code change): §1's original module layout named `prompts/*.tmpl` (embedded via
  `go:embed`, versioned). That file/directory was never built. Every Advisor prompt
  (`jobs/triage.go`, `jobs/followup.go`, and any FIX-side prompt text) lives as an inline Go string
  const in the package that uses it. This is a **deliberate, accepted divergence**, not a gap:
  §7's observability requirement — "prompt identity = sha256 of the fully rendered prompt plus the
  template version" — is satisfied exactly as written regardless of whether the pre-render template
  came from an embedded `.tmpl` file or a Go const: the hash is computed over the fully rendered
  prompt text at call time either way, and "the template version" is free to be (and is) a version
  identifier attached to the const, not a filesystem artifact. `go:embed` templates would add a
  build-time indirection (load file → render) for zero behavioral gain over `const → render`, at
  the cost of a second thing to keep in sync with the code that renders it. Revisit only if a
  future requirement needs prompt text to be edited without a rebuild (e.g. hot-reloadable
  prompts) — nothing in the current plan needs that.
...
## 12. Deployability matrix (N8g close-out, S3-durability corrected N8h, FIX-executor BYOK + boot-recovery corrected N8i)
...
| State durability | emptyDir-equivalent named volumes + S3 snapshots (§2.8) — **no PVC**. Snapshot lifecycle is now real and wired as of **N8h** (restore-on-empty-startup, periodic, post-compaction, SIGTERM-upload), proven through `runApp` + a SigV4 fake; a real cluster/minio round-trip stays deferred | emptyDir + S3 snapshots — **no PVC** (helm template renders zero worker PVCs) |
...
| FIX executor BYOK creds | **`WORKER_FIX_EXECUTOR_ENV`** (N8i, comma/newline `KEY=VALUE`/bare-`KEY` list) is the ONLY channel to `$FIX_EXECUTOR_CMD`'s child env — forbidden-key guarded (never Sentinel/LLM/git-token keys); in `.env.example`, addable to the compose `environment:` map / Helm `worker.env{}` | same env var via `worker.env{}` |
| Crash-recovery bounded | in-flight FIX resume runs **async** (`DispatchResume` goroutine, FIX-timeout bounded) so it never blocks the poll loop / `/readyz` (N8i regression fix); non-FIX in-flight jobs replay their journaled batch **verbatim** (Advisor never re-consulted) | same binary |
